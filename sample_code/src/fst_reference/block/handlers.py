...
def _unsupported_block_handler(payload: bytes, idx: int, block_str: str, offset: int, output_dir: str):
    raise RuntimeError(f"Block type {block_str} at offset {offset} is recognized but not yet supported in this parser.")
...
BLOCKS = {
    ...
    int(BlockType.VCDATA_DYN_ALIAS): {'name': 'VCDATA_DYN_ALIAS', 'handler': _unsupported_block_handler},
    int(BlockType.VCDATA_DYN_ALIAS2): {'name': 'VCDATA_DYN_ALIAS2', 'handler': CallVCDATA},
}
