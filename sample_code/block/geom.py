...
def CallGEOM(payload: bytes, idx: int, block_str: str, offset: int, output_dir: str):
    ...
    if payload_len < 16:
        raise RuntimeError(f"CallGEOM: payload size shall be at least 16, but got {payload_len}")
    ...
    if actual_uncompressed_length != uncompressed_length:
        raise RuntimeError(f"CallGEOM: declared uncompressed length {uncompressed_length} does not match actual {actual_uncompressed_length}")
    ...
    if dec:
        ...
        while br2.remaining() > 0 and len(values) < count:
            ...
            values.append(v)
        if len(values) != count:
            raise RuntimeError(f"CallGEOM: parsed {len(values)} values but expected {count}")
